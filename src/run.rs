...
impl State {
    ...
    async fn new_windowed(window: &Window) -> Self {
        ...
        let (compute_texture_size, compute_texture_desc, compute_texture, compute_texture_view) =
            Self::get_compute_texture(&device);

        let (bind_group, bind_group_layouts, stuff_buffer, compute_buffer) = Self::get_bind_group(
            &device,
            &screen_buffer,
            &Stuff::new(),
    ...
    async fn new_windowless() -> Self {
        ...
        stuff.windowless = 1;
        let (bind_group, bind_group_layouts, stuff_buffer, compute_buffer) =
            Self::get_bind_group(&device, &screen_buffer, &stuff, &compute_texture_view);
    ...
    fn get_screen_texture<'a, 'b>(
        device: &'a wgpu::Device,
    ) -> (
        (u32, u32),
        wgpu::TextureDescriptor<'b>,
        wgpu::Texture,
        wgpu::TextureView,
    ) {
    ...
    fn get_bind_group(
        ...
        let stuff_buffer = device.create_buffer_init(&wgpu::util::BufferInitDescriptor {
        ...
        let bind_group_layouts =
            device.create_bind_group_layout(&wgpu::BindGroupLayoutDescriptor {
                ...
                entries: &[
                    ...
                    wgpu::BindGroupLayoutEntry {
                        // https://sotrh.github.io/learn-wgpu/beginner/tutorial5-textures/#shader-time
    ...
    fn reset_buffers(&mut self, reset_screen_buffer: bool) {
        if reset_screen_buffer {
            ...
            let (_, _, compute_texture, compute_texture_view) =
                Self::get_compute_texture(&self.device);
    ...
    fn dump_compute_texture(&mut self) {
        ...
        let output_buffer_size =
            (4 * u32_size * self.compute_texture_size.0 * self.compute_texture_size.1)
        ...
        // unmap the buffer
        {
            ...
            let buffer = ImageBuffer::<Rgba<u8>, _>::from_raw(
                self.compute_texture_size.0,
                self.compute_texture_size.1,
                data,
            )
            .unwrap();
    ...
    fn dump_render(&mut self) {
        ...
        self.queue
            .write_buffer(&self.stuff_buffer, 0, bytemuck::cast_slice(&[self.stuff]));
                        ...
                        match k {
                            ...
                            VirtualKeyCode::P => {
                                match self.active_shader {
                                    ...
                                    ActiveShader::Mandlebrot => {
                                        // since resolution cannot be increased anyway, do not render
                            ...
                            VirtualKeyCode::Key1 => {
                                ...
                                self.importer =
                                    shader_importer::Importer::new(&self.active_shader.to_string());
                                if self.compile() {
    ...
    fn compile(&mut self) -> bool {
        ...
        if shader_code.is_none() {
            return true;
        }
        if !self.compile_status && self.shader_code == shader_code {
            return true;
        }
        ...
        if self.importer.work_group_count.is_some() {
            ...
            if work_group_count != self.work_group_count {
                dbg!(format!(
                    "work_group count changed from {} to {}",
    ...
    fn compile_render_shaders(&mut self) -> bool {
        ...
        let format = {
            if self.stuff.windowless != 1 {
                self.config.as_ref().unwrap().format
            } else {
                self.screen_texture_desc.as_ref().unwrap().format
            }
        };
    ...
    fn execute_render_and_compute_pass(
        &self,
    ...
    async fn render_windowless(&mut self) {
        ...
        let output_buffer_size = (u32_size
            * self.screen_texture_size.as_ref().unwrap().0
            * self.screen_texture_size.as_ref().unwrap().1)
            as wgpu::BufferAddress;
        let output_buffer_desc = wgpu::BufferDescriptor {
            size: output_buffer_size,
            usage: wgpu::BufferUsages::COPY_DST
        ...
        self.execute_render_and_compute_pass(
            &mut encoder,
            self.screen_texture_view.as_ref().unwrap(),
        );
        ...
        // unmap the buffer
        {
            ...
            let buffer = ImageBuffer::<Rgba<u8>, _>::from_raw(
                self.screen_texture_size.unwrap().0,
                self.screen_texture_size.unwrap().1,
                data,
            )
            .unwrap();
            buffer.save(file_name()).unwrap();
...
pub fn window_event_loop() {
    ...
    event_loop.run(move |event, _, control_flow| {
        match event {
            // https://docs.rs/winit/0.25.0/winit/event/enum.WindowEvent.html
            Event::WindowEvent {
                ref event,
                window_id,
...
fn file_name() -> String {
    ...
    if std::path::Path::new(&path).exists() {
        // should never happen, but i dont wanna loose files
